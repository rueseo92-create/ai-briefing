...
export function PostCard({ post, compact, locale = defaultLocale }: PostCardProps) {
  ...
  if (compact) {
    return (
      <a href={lh(`/posts/${post.slug}`)} className="group block overflow-hidden rounded-xl bg-white border border-slate-200/80 hover:shadow-lg hover:shadow-primary/5 transition-all">
        {post.thumbnail && (
        ...
        <div className="p-4">
          <h3 className="text-sm font-bold text-on-surface group-hover:text-primary transition-colors line-clamp-2 font-headline">{post.title}</h3>
  ...
  return (
    <a href={lh(`/posts/${post.slug}`)} className="group block overflow-hidden rounded-xl bg-white border border-slate-200/80 hover:shadow-xl hover:shadow-primary/5 hover:border-primary/20 transition-all duration-300">
      {post.thumbnail ? (
        ...
        <div className="aspect-video bg-gradient-to-br from-indigo-50 to-cyan-50 flex items-center justify-center">
          <span className="text-4xl opacity-40">{category?.emoji || "🤖"}</span>
        </div>
      )}
      <div className="p-6">
        <div className="mb-3 flex items-center gap-2 flex-wrap">
          ...
          <time className="text-xs text-on-surface-variant tracking-wide ml-auto">{post.date}</time>
        </div>
        <h3 className="mb-2 text-lg font-extrabold text-on-surface group-hover:text-primary transition-colors line-clamp-2 leading-snug font-headline">{post.title}</h3>
        <p className="text-sm text-on-surface-variant line-clamp-2 leading-relaxed">{post.description}</p>
        {post.tags.length > 0 && (
          <div className="mt-4 flex flex-wrap gap-1.5">
            {post.tags.slice(0, 3).map((tag) => (
              <span key={tag} className="px-2.5 py-1 bg-slate-50 border border-slate-100 rounded-md text-xs text-on-surface-variant">#{tag}</span>
            ))}
          </div>
        )}
      </div>
